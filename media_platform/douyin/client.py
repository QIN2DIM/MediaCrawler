...
class DOUYINClient(AbstractApiClient):
    ...
    async def search_info_by_keyword(
        ...
        referer_url += (
            f"?publish_time={publish_time.value}&sort_type={sort_type.value}&type=general"
        )
    ...
    async def get_sub_comments(self, comment_id: str, cursor: int = 0):
        ...
        params = {"comment_id": comment_id, "cursor": cursor, "count": 20, "item_type": 0}
        keywords = request_keyword_var.get()
        referer_url = (
            "https://www.douyin.com/search/"
            + keywords
    ...
    async def get_all_user_aweme_posts(self, sec_user_id: str, callback: Optional[Callable] = None):
        ...
        while posts_has_more == 1:
            ...
            aweme_list = (
                aweme_post_res.get("aweme_list") if aweme_post_res.get("aweme_list") else []
            )
            utils.logger.info(
